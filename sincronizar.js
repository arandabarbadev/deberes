...
const CLAVE_AJUSTES = 'deberesAjustes';
...
async function subirADatosJson(textoPlano, nuestroModificado) {
  ...
  const respuestaAmable = codigo => {
    ...
    if (codigo === 404) return 'el token no ve este repo: en el token, Repository access → Only select repositories → deberes';
  ...
  const respuesta = await fetch(URL_API, {
    ...
    body: JSON.stringify({
      message: 'Actualizar datos de Mis deberes',
...
// espera unos segundos por si haces más cambios y entonces sube
...
async function subirCambios() {
  ...
  ocupado = true;
  try {
    const paquete = await cifrar({ tareas: datos.tareas }, contraseña);
...
async function sincronizarAlIniciar() {
  ...
  try {
    ...
    datos.tareas = Array.isArray(nuevo.tareas) ? nuevo.tareas : [];
